...
/**
 * A Layout that arranges its children in a single column and a single row. The direction of
 ...
 * {@link LayoutParams LayoutParams}.
 * The default orientation is horizontal.
 *
 * <p>
 * Also see {@link LayoutParams LayoutParams}
 * for layout attributes </p>
 *
 * @attr ref com.jun.tools.R.styleable#JunLinearLayout_baselineAlignedJun
 * @attr ref com.jun.tools.R.styleable#JunLinearLayout_baselineAlignedChildIndexJun
 * @attr ref com.jun.tools.R.styleable#JunLinearLayout_gravityJun
 * @attr ref com.jun.tools.R.styleable#JunLinearLayout_measureWithLargestChildJun
 * @attr ref com.jun.tools.R.styleable#JunLinearLayout_orientationJun
 * @attr ref com.jun.tools.R.styleable#JunLinearLayout_weightSumJun
 */
@RemoteView
public class JunLinearLayout extends ViewGroup {
    ...
    public JunLinearLayout(Context context) {
        super(context);
    }

    public JunLinearLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public JunLinearLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        TypedArray a = context.obtainStyledAttributes(attrs,
                R.styleable.JunLinearLayout, defStyle, 0);

        int index = a.getInt(R.styleable.JunLinearLayout_orientationJun, -1);
        if (index >= 0) {
            setOrientation(index);
        }

        index = a.getInt(R.styleable.JunLinearLayout_gravityJun, -1);
        if (index >= 0) {
            setGravity(index);
        }

        boolean baselineAligned = a.getBoolean(R.styleable.JunLinearLayout_baselineAlignedJun, true);
        if (!baselineAligned) {
            setBaselineAligned(baselineAligned);
        }

        mWeightSum = a.getFloat(R.styleable.JunLinearLayout_weightSumJun, -1.0f);

        mBaselineAlignedChildIndex =
                a.getInt(R.styleable.JunLinearLayout_baselineAlignedChildIndexJun, -1);

        mUseLargestChild = a.getBoolean(R.styleable.JunLinearLayout_measureWithLargestChildJun, false);

        setDividerDrawable(a.getDrawable(R.styleable.JunLinearLayout_dividerJun));
        mShowDividers = a.getInt(R.styleable.JunLinearLayout_showDividersJun, SHOW_DIVIDER_NONE);
        mDividerPadding = a.getDimensionPixelSize(R.styleable.JunLinearLayout_dividerPaddingJun, 0);
...
//    @android.view.RemotableViewMethod
    ...
    public LayoutParams generateLayoutParams(AttributeSet attrs) {
        return new JunLinearLayout.LayoutParams(getContext(), attrs);
    }
    ...
    public static class LayoutParams extends ViewGroup.MarginLayoutParams {
        ...
        public LayoutParams(Context context, AttributeSet attrs) {
            ...
            weight = a.getFloat(0, 0);
            gravity = a.getInt(1, -1);
        ...
        public String debug(String output) {
            return output + "JunLinearLayout.LayoutParams={width=" + sizeToString(width) +
        ...
        private String sizeToString(int size){
            ...
            if (size == MATCH_PARENT){
                strSize = "MATCH_PARENT";
            } else if (size == WRAP_CONTENT){
                strSize = "WRAP_CONTENT";
            } else if (size == FILL_PARENT){
                strSize = "FILL_PARENT";
            } else if (size == FILL_PARENT){
                strSize = "FILL_PARENT";
            } else {
    ...
    protected boolean isLayoutRtl(){
        ...
        try {
            String name = "isLayoutRtl";
            Method method = getClass().getMethod(name, boolean.class);
            isRtl = (boolean)method.invoke(this);
        }catch (Exception e){
    ...
    protected int getResolvedLayoutDirection(){
        int isRtl = View.LAYOUT_DIRECTION_LTR;

        try {
            String name = "getResolvedLayoutDirection";
            Method method = getClass().getMethod(name, boolean.class);
            isRtl = (int)method.invoke(this);
        }catch (Exception e){
